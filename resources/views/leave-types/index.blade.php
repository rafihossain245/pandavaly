...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Leave Types List
                </h2>
                <button class="btn btn-primary create-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
                    <i class="fas fa-plus mr-2"></i>Add New Leave Types
                </button>
            </div>
                    ...
                    <table class="table table-hover min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Max Leave</th>                                                                                                                      
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($datas as $key => $value)                       
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->max_leaves_count }}</td>        
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            <button class="btn btn-outline-primary edit-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200" 
                                                ...
                                                data-item_max_leaves_count="{{ $value->max_leaves_count }}"                                                 
                                                data-action="{{ route('role.leave-types.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'leave_type' => $value->id]) }}" title="Edit Item">
    ...
    @include('leave-types.create-modal')
    @include('leave-types.edit-modal')
    @include('leave-types.delete-modal')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.edit-btn').click(function() {
                ...
                const item_max_leaves_count = $(this).data('item_max_leaves_count');
                
                // Set values in the edit form
                $('#editItemId').val(item_id);
                $('#edit_name').val(item_name);
                $('#edit_max_leaves_count').val(item_max_leaves_count);                
        ...
        function validateCreateForm() {
            ...
            if (!$('#create_max_leaves_count').val().trim() || ($('#create_max_leaves_count').val().trim() < 1)) {
                $('#create_max_leaves_count').next('.error-message').removeClass('hidden');
                $('#create_max_leaves_count').addClass('border-red-500');
                isValid = false;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.leave-types.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection