...
@section('main-content')
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Sliders List
                </h2>
                <button
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($datas as $key => $value)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->link }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap"><img src="{{ asset($value->image_path) }}"
                                            alt="" width="120"></td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            <button
                                                class="btn btn-outline-primary edit-item-btn border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white px-3 py-1 rounded-md transition duration-200"
                                                data-item_id="{{ $value->id }}" data-title="{{ $value->title }}"
                                                {{-- was data-name previously --}} data-link="{{ $value->link }}"
                                                data-image="{{ asset($value->image_path) }}" {{-- send absolute URL for preview --}}
                                                data-is_active="{{ (int) $value->is_active }}" title="Edit Item">
    ...
    @include('sliders.create-modal')
    @include('sliders.edit-modal')
    @include('sliders.delete-modal')
@endsection
@section('js')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function(e) {
                e.preventDefault();
                window.location =
                    '{{ route('role.brands.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection
