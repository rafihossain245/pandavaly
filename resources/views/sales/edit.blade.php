        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 pb-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-edit mr-2"></i> Edit Sale #{{ $sale->invoice_no }}
                </h2>
                <a href="{{ route('role.sales.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}" class="btn btn-primary create-new-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition duration-200">
            ...
            <div class="states-table-content" style="padding: 15px;">
                <div class="sale-modal-body modal-body overflow-y-auto mt-2">
                    <form class="closest" id="saleEditForm" action="{{ route('role.sales.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'sale' => $sale->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4" style="justify-content: center">   
                            ...
                            <div class="mb-2">
                                ...
                                <select id="customer_id" name="customer_id" class="form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 select2" style="width: 100%">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="date" id="sale_date" required value="{{ $sale->sale_date }}" name="sale_date" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="mb-2">
                                <label for="sale_date" class="block text-gray-700 text-sm font-bold mb-2">Invoice No.</label>
                                <input type="text" readonly value="{{ $sale->invoice_no }}" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>
                                    ...
                                    <div class="md:col-span-5">
                                        ...
                                        <select id="isProductSelected" onchange="selectProduct(this)" class="select2 form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                                            ...
                                            @foreach ($products as $product)
                                                <option value="{{ $product->id }}" data-name="{{ $product->name }}" data-sku="{{ $product->sku }}" data-selling_price="{{ $product->selling_price }}" {{ empty($product->stock_qty) ? 'disabled' : '' }}>{{ $product->name }} => {{ $product->sku }} {{ empty($product->stock_qty) ? '[Out Of Stock]' : '' }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="calculation-area mb-6">
                            <div style="display: flex; justify-content: space-between; gap: 15px">    
                                ...
                                <div class="selected-products space-y-3" id="appendProducts" style="width: 65%; max-height: 424px; overflow-y: auto; scrollbar-width: thin;">                                    
                                    @forelse ($sale->items as $key => $detail)
                                    @php
                                        $appendClass = '';
                                        if ($key % 2 === 0) {
                                            $appendClass = 'blue';
                                        ...
                                        <div class="product-card card_no{{ $detail->product_id }} bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex justify-between items-center">
                                            ...
                                            <div class="flex items-center space-x-6">
                                                ...
                                                <div class="text-center" style="margin: 0">
                                                    ...
                                                    <p class="font-medium text-blue-600">৳<span class="item_Subtotal">{{ $detail->subtotal }}</span></p>
                                                    <input type="hidden" name="subtotal[]" class="itemSubtotal" value="{{ $detail->subtotal }}">
                                                </div>
                                ...
                                <div class="calculation-card p-5" style="width: 35%">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <div class="relative">
                                                ...
                                                <input type="number" onblur="calculateGrandTotal()" step="0.01" value="{{ $sale->paid_amount }}" id="paid_amount" name="paid_amount" class="form-input w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" style="background: white">
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="relative">
                                                ...
                                                <input type="text" id="due_amount" value="{{ $sale->due_amount }}" name="due_amount" readonly class="form-input w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md bg-gray-100" style="background: white">
                                            </div>
                                        </div>                                        
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <select name="payment_method" id="payment_method" class="form-control select2" style="width: 100%">
                                                    ...
                                                    <option value="advance" {{ ($sale->payment_method == 'advance') ? 'selected' : '' }}>Advance</option>
                                                    <option value="checque" {{ ($sale->payment_method == 'checque') ? 'selected' : '' }}>Checque</option>
                                        ...
                                        <div>
                                            ...
                                            <select name="bank_id" id="bank_id" class="form-control select2" style="width: 100%">
                                                ...
                                                @foreach ($banks as $bank)                                                    
                                                    <option value="{{ $bank->id }}" {{ $sale->bank_id && ($sale->bank_id == $bank->id) ? 'selected' : '' }}>{{ $bank->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-4">                                        
                                        <div class="border-t border-gray-300 pt-3 flex justify-between items-center" style="border-top-width: 2px;">
                                            ...
                                            <span class="amount-highlight">৳ <span class="sub_total">{{ $sale->items->sum('subtotal') }}</span></span>                                            
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-gray-800 font-semibold">Grand Total:</span>
                                            <span class="text-xl font-bold text-blue-600">৳ <span class="total_amount">{{ $sale->total_amount }}</span></span>
...
@section('js')
    ...
    <script>
        ...
        $('#editSaleBtn').click(function(e) {
            ...
            if (!$('#customer_id').val() || ($('#customer_id').val() == '') || ($('#customer_id').val() == null)) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
                    ...
                    $('#appendProducts').prepend(`
                        <div class="product-card card_no${itemId} bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex justify-between items-center">
                            <div class="flex items-start">
                                <div class="bg-${appendClass}-100 rounded-lg flex items-center justify-center mr-4" style="width: 70px; height: 45px; margin-right: 10px;">
                                    <i class="fas fa-box text-${appendClass}-500"></i>
                                </div>
                                <div>
