        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 pb-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-edit mr-2"></i> Edit Purchase #{{ $purchase->purchase_no }}
                </h2>
            ...
            <div class="states-table-content" style="padding: 15px;">
                <div class="common-modal-body modal-body overflow-y-auto mt-2">
                    <form class="closest" id="purchaseEditForm" action="{{ route('role.purchases.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'purchase' => $purchase->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4" style="justify-content: center">   
                            ...
                            <div class="mb-2">
                                ...
                                <input type="date" id="purchase_date" required value="{{ $purchase->purchase_date }}" name="purchase_date" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="mb-2">
                                <label for="purchase_date" class="block text-gray-700 text-sm font-bold mb-2">Purchase No.</label>
                                <input type="text" readonly value="{{ $purchase->purchase_no }}" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>
                        ...
                        <div class="calculation-area mb-6">
                            <div style="display: flex; justify-content: space-between; gap: 15px">    
                                ...
                                <div class="selected-products space-y-3" id="appendProducts" style="width: 65%; max-height: 424px; overflow-y: auto; scrollbar-width: thin;">                                    
                                    @forelse ($purchase->items as $key => $detail)
                                    @php
                                        $appendClass = '';
                                        if ($key % 2 === 0) {
                                            $appendClass = 'blue';
                                ...
                                <div class="calculation-card p-5" style="width: 35%">
                                    ...
                                    <div class="space-y-4">
                                        <div>
                                            ...
                                            <div class="relative">
                                                ...
                                                <input type="number" onblur="calculateGrandTotal()" step="0.01" value="{{ $purchase->paid_amount }}" id="paid_amount" name="paid_amount" class="form-input w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.00" style="background: white">
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="relative">
                                                ...
                                                <input type="text" id="due_amount" value="{{ $purchase->due_amount }}" name="due_amount" readonly class="form-input w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md bg-gray-100" style="background: white">
                                            </div>
                                        </div>                                        
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <select name="payment_method" id="payment_method" class="form-control select2" style="width: 100%">
                                                    ...
                                                    <option value="advance" {{ ($purchase->payment_method == 'advance') ? 'selected' : '' }}>Advance</option>
                                                    <option value="checque" {{ ($purchase->payment_method == 'checque') ? 'selected' : '' }}>Checque</option>
                                        ...
                                        <div>
                                            ...
                                            <select name="bank_id" id="bank_id" class="form-control select2" style="width: 100%">
                                                ...
                                                @foreach ($banks as $bank)                                                    
                                                    <option value="{{ $bank->id }}" {{ $purchase->bank_id && ($purchase->bank_id == $bank->id) ? 'selected' : '' }}>{{ $bank->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-4">                                        
                                        <div class="border-t border-gray-300 pt-3 flex justify-between items-center" style="border-top-width: 2px;">
                                            ...
                                            <span class="amount-highlight">৳ <span class="sub_total">{{ $purchase->items->sum('subtotal') }}</span></span>                                            
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-gray-800 font-semibold">Grand Total:</span>
                                            <span class="text-xl font-bold text-blue-600">৳ <span class="total_amount">{{ $purchase->total_amount }}</span></span>
                        ...
                        <div class="flex justify-end pt-2">
                            <button id="editpurchaseBtn" type="button" class="btn btn-success px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition duration-200" style="cursor: pointer">
                                Submit
                            </button>
                        </div>
                ...
                success: function (response) {
                    if (response.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Done',
                            text: 'purchase edited successfully!'
                        });                            
