                            ...
                            <div class="closest filter-row">   
                                <div class="filter-group">
                                    ...
                                    <select id="reason" name="reason" class="form-control select2" style="width: 100%">
                                        ...
                                        <option value="purchase_receipt" {{ request('reason') == 'purchase_receipt' ? 'selected' : '' }}>Purchase Receipt</option>
                                        <option value="sale_dispatch" {{ request('reason') == 'sale_dispatch' ? 'selected' : '' }}>Sale Dispatch</option>
                                        <option value="transfer" {{ request('reason') == 'transfer' ? 'selected' : '' }}>Transfer</option>
                                        <option value="adjustment" {{ request('reason') == 'adjustment' ? 'selected' : '' }}>Adjustment</option>
                                        <option value="return_in" {{ request('reason') == 'return_in' ? 'selected' : '' }}>Return In</option>
                                        <option value="return_out" {{ request('reason') == 'return_out' ? 'selected' : '' }}>Return Out</option>
                                    </select>
                                </div>
                                <div class="filter-group">
                                    ...
                                    <select id="warehouse_id" name="warehouse_id" class="form-control select2" style="width: 100%">
                                        ...
                                        @foreach ($warehouses as $wh)
                                            <option value="{{ $wh->id }}" {{ $wh->id == request('warehouse_id') ? 'selected' : '' }}>{{ $wh->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="filter-group">
                    ...
                    <table class="table table-hover min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Reason / Ref</th>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            ...
                                            $badgeColor = in_array($value->reason, $inReasons)
                                                ? 'bg-green-100 text-green-700 border-green-300'
                                                : 'bg-red-100 text-red-700 border-red-300';
                                        @endphp
                                        <span class="inline-block px-2 py-1 text-xs font-semibold rounded-full border {{ $badgeColor }}">
                                            {{ strtoupper(str_replace('_', ' ', $value->reason)) }}
                                        </span>
                                        <br>
                                        <small class="text-gray-500">{{ $value->ref_type }} {{ $value->ref_id }}</small>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->productSku?->product?->name }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->qty }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->fromWarehouse?->name ?? '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">{{ $value->toWarehouse?->name ?? '—' }}</td>                                 
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="btn-group btn-group-sm flex space-x-1">
                                            ...
                                            <button class="btn btn-outline-danger border border-red-500 text-red-500 hover:bg-red-500 hover:text-white px-3 py-1 rounded-md transition duration-200" onclick="confirmDelete('{{ $value->id }}', '{{ $value->ref_id }}')">
