        ...
        <div class="states-table-container">
            ...
            <div class="states-table-content" style="padding: 15px;">
                <div class="common-modal-body modal-body overflow-y-auto mt-2">
                    <form class="closest" id="dataCreateForm" action="{{ route('role.stock-movements.store', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4" style="justify-content: center">
                            <div class="mb-2">
                                ...
                                <select id="reason" name="reason" class="form-control select2" style="width: 100%">
                                    ...
                                    <option value="sale_dispatch">Sale Dispatch (Stock Out)</option>
                                    <option value="adjustment">Stock Adjustment (In)</option>
                                    <option value="return_in">Return In</option>
                                    <option value="return_out">Return Out (Stock Out)</option>
                                </select>
                            </div>
                            <div class="mb-2">
                            ...
                            <div class="mb-2">
                                <label for="ref_id" class="block text-gray-700 text-sm font-bold mb-2">Reference No. <small class="text-gray-400 font-normal">(PO/SO/GRN)</small></label>
                                <input type="text" id="ref_id" name="ref_id" placeholder="e.g. SO-20260608-1234" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="mb-2">
                                <label class="block text-sm text-gray-700 font-bold mb-1">Note</label>
                                <textarea name="note" class="w-full h-10 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" placeholder="Write ..." style="background: white;"></textarea>
                            </div>
                        </div>                                                                     
                        
                        <div class="flex justify-end pt-2" style="align-items: center">                            
                            <button id="createDataBtn" type="button" class="btn btn-success px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition duration-200" style="cursor: pointer">
                                Submit
                            </button>
                        </div>
...
@section('js')
    ...
    <script>
        ...
        $('#createDataBtn').click(function(e) {
            e.preventDefault();
            ...
            let formData = new FormData($('#dataCreateForm')[0]);
            $.ajax({
                url: $('#dataCreateForm').attr('action'),
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
