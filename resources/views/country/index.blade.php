...
<style>
    ...
    .admin-stats-grid .admin-stat-card.danger {
        /* background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); */
        background: #f4f4f4;
        color: red;
    }
    ...
    <div class="admin-stats-grid">
        <div class="admin-stat-card primary">
            <div class="position-relative">
                <div class="admin-stat-value">{{ $totalCountries }}</div>
                <div class="admin-stat-label">Total Countries</div>
        ...
        <div class="admin-stat-card success">
            <div class="position-relative">
                <div class="admin-stat-value">{{ $activeCountries }}</div>
                <div class="admin-stat-label">Active Countries</div>
                <div class="admin-stat-icon">
        ...
        <div class="admin-stat-card warning">
            <div class="position-relative">
                <div class="admin-stat-value">{{ $inactiveCountries }}</div>
                <div class="admin-stat-label">Inactive Countries</div>
                <div class="admin-stat-icon">
        ...
        <div class="admin-stat-card danger">
            <div class="position-relative">
                <div class="admin-stat-value">{{ $trashedCountries }}</div>
                <div class="admin-stat-label">Trashed Countries</div>
                <div class="admin-stat-icon">
        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 py-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-list mr-2"></i>Country List
                </h2>
            </div>
                        ...
                        <div class="filter-content">
                            <div class="filter-row">
                                <div class="filter-group">
                                    ...
                                    <input type="text" name="country_name" id="country_name" value="{{ request('country_name') }}" placeholder="Type Country Name...">  
                                </div>
                                <div class="filter-group">
                                    <label for="status">Status</label>
                                    <select id="status" name="status" class="form-control select2">
                                        <option value="all" {{ request('status') === null || request('status') === '' ? 'selected' : '' }}>All Statuses</option>
                            ...
                            @forelse ($countries as $key => $country)                       
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap" style="padding-left: 20px">
                                        <strong>{{ ($countries->currentPage() - 1) * $countries->perPage() + $key + 1 }}</strong>
                                    </td>                                    
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <small class="text-gray-500">{{ date('M d, Y', strtotime($country->created_at)) }}</small>
                                    </td>
                                </tr>
                            @empty
                            <tr>
                                <td colspan="7" class="text-center py-8">
                                    ...
                                    <h4 class="text-gray-500 text-xl font-medium mb-2">No Countries Found!</h4>
    ...
    <script>        
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.countries.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
@endsection