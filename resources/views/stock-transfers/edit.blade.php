        ...
        <div class="states-table-container">
            <div class="states-table-header bg-blue-600 px-6 pb-4 flex justify-between items-center">
                <h2 class="states-table-title text-white text-xl font-semibold" style="color: white">
                    <i class="fas fa-edit mr-2"></i> Edit Transfer No #{{ $transfer->transfer_no }}
                </h2>
            ...
            <div class="states-table-content" style="padding: 15px;">
                <div class="common-modal-body modal-body overflow-y-auto mt-2">
                    <form class="closest" id="transferEditForm" action="{{ route('role.stock-transfers.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'stock_transfer' => $transfer->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4" style="justify-content: center">   
                            <input type="hidden" name="transfer_id" value="{{ $transfer->id }}">
                            <div class="mb-2">
                                <label for="from_warehouse_id" class="block text-gray-700 text-sm font-bold mb-2">From Warehouse</label>
                                <select id="from_warehouse_id" name="from_warehouse_id" class="form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 select2" style="width: 100%">
                                    @foreach ($warehouses as $wh)
                                        <option value="{{ $wh->id }}" {{ $transfer->from_warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }} ({{ $wh->code }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-2">
                                <label for="to_warehouse_id" class="block text-gray-700 text-sm font-bold mb-2">To Warehouse</label>
                                <select id="to_warehouse_id" name="to_warehouse_id" class="form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 select2" style="width: 100%">
                                    @foreach ($warehouses as $wh)
                                        <option value="{{ $wh->id }}" {{ $transfer->to_warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }} ({{ $wh->code }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-2">
                                <label for="transfer_no" class="block text-gray-700 text-sm font-bold mb-2">Transfer No.</label>
                                <input type="text" readonly value="{{ $transfer->transfer_no }}" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                        </div>                                                                     
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4" style="justify-content: center">                               
                            <div class="mb-2">
                                <label class="block text-sm text-gray-700 font-bold mb-1">Transfer Product</label>
                                <select id="product_id" name="product_id" class="select2 form-select w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                                    ...
                                    @foreach ($products as $product)
                                        <option value="{{ $product->id }}" {{ $transfer->product_id == $product->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-2">
                                ...
                                <input type="number" id="qty" min="0.01" step="0.01" value="{{ $transfer->qty }}" name="qty" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div class="mb-2">
                                <label class="block text-sm text-gray-700 font-bold mb-1">Additional Note</label>
                                <div class="space-y-4">
                                    <textarea 
                                    name="note" 
                                    class="w-full h-10 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none" 
                                    placeholder="Write ..." style="background: white;">{{ $transfer->note }}</textarea>
                                </div>
                            </div>
                        </div>                                                                     
                        
                        <div class="flex justify-end pt-2" style="align-items: center">                            
                            <button id="edittransferBtn" type="button" class="btn btn-success px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition duration-200" style="cursor: pointer">
                                Submit
                            </button>
                        </div>
...
@section('js')
    ...
    <script>
        ...
        $('#edittransferBtn').click(function(e) {
            e.preventDefault();
            ...
            if (from_warehouse_id == to_warehouse_id) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
                    text: 'Source and destination warehouse cannot be the same!'
                });
                return;
            }
            ...
            let prodQty = $('#quantity').val();  
            if (!prodQty || (prodQty < 1)) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
                    text: 'Minimum transfer quantity required!'
                });               
                return;
            }

            let formData = new FormData($('#transferEditForm')[0]);
            $.ajax({
                url: $('#transferEditForm').attr('action'),
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
