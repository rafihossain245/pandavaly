...
@section('main-content')
                        ...
                        <div class="filter-content">
                            <div class="closest filter-row">                             
                                ...
                                <div class="filter-group">
                                    ...
                                    <input type="text" name="email" value="{{ request('email') }}" id="email" class="form-control" placeholder="Enter Email">
                                </div>
                                <div class="filter-group">
                                    <label for="phone">Phone</label>
                                    <input type="text" name="phone" value="{{ request('phone') }}" id="phone" class="form-control" placeholder="Enter phone">
                                </div>
                            </div>
    ...
    @include('suppliers.delete-modal')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelector('.reset-btn').addEventListener('click', function (e) {
                e.preventDefault();
                window.location = '{{ route('role.suppliers.index', ['role' => Str::slug(Auth::user()->getRoleNames()->first())]) }}';
            });
        });
    </script>
    <script>
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById('addBankRow').addEventListener('click', function() {
        const tableBody = document.getElementById('bankTableBody');
        const newRow = document.createElement('tr');
        newRow.innerHTML = `
            <td class="border p-2"><input type="text" name="bank_accounts[${rowIndex}][bank_name]" class="w-full border rounded-md px-2 py-1" placeholder="Bank Name"></td>
            <td class="border p-2"><input type="text" name="bank_accounts[${rowIndex}][account_name]" class="w-full border rounded-md px-2 py-1" placeholder="Account Name"></td>
            <td class="border p-2"><input type="text" name="bank_accounts[${rowIndex}][account_number]" class="w-full border rounded-md px-2 py-1" placeholder="Account Number"></td>
            <td class="border p-2"><input type="text" name="bank_accounts[${rowIndex}][branch]" class="w-full border rounded-md px-2 py-1" placeholder="Branch"></td>
            <td class="border p-2"><input type="text" name="bank_accounts[${rowIndex}][swift]" class="w-full border rounded-md px-2 py-1" placeholder="Routing No."></td>
...
document.addEventListener('DOMContentLoaded', function() {
    // Add new bank row
    let editBankIndex = 0;
    document.getElementById('addEditBankRow').addEventListener('click', function() {
        let tbody = document.getElementById('editBankTableBody');
        let row = `
            <tr>
                <td class="border p-2"><input type="text" name="bank_accounts[${editBankIndex}][bank_name]" class="w-full border rounded-md px-2 py-1" placeholder="Bank Name"></td>
                <td class="border p-2"><input type="text" name="bank_accounts[${editBankIndex}][account_name]" class="w-full border rounded-md px-2 py-1" placeholder="Account Name"></td>
                <td class="border p-2"><input type="text" name="bank_accounts[${editBankIndex}][account_no]" class="w-full border rounded-md px-2 py-1" placeholder="Account Number"></td>
                <td class="border p-2"><input type="text" name="bank_accounts[${editBankIndex}][branch]" class="w-full border rounded-md px-2 py-1" placeholder="Branch"></td>
                <td class="border p-2"><input type="text" name="bank_accounts[${editBankIndex}][swift]" class="w-full border rounded-md px-2 py-1" placeholder="Routing No."></td>
                <td class="border p-2 text-center">
                    <button type="button" class="removeEditRow bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded">X</button>
                </td>
            </tr>`;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $(document).on('click', '.edit-item-btn', function () {
        let supplierId = $(this).data('item_id');
        ...
        // Clear previous rows
        $('#editBankTableBody').empty();

        // Generate route dynamically using Laravel helper
        let bankAccountsUrl = "{{ route('role.suppliers.bankAccounts', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'id' => '__id__']) }}";
        bankAccountsUrl = bankAccountsUrl.replace('__id__', supplierId);

        // Fetch supplier bank accounts via AJAX
        $.ajax({
            url: bankAccountsUrl,
            type: 'GET',
            success: function (res) {
                if (res.success && res.data.length > 0) {
                    res.data.forEach(function (acc, i) {
                        let row = `
                            <tr>
                                <td class="border p-2"><input type="text" name="bank_accounts[${i}][bank_name]" value="${acc.bank_name}" class="w-full border rounded-md px-2 py-1"></td>
                                <td class="border p-2"><input type="text" name="bank_accounts[${i}][account_name]" value="${acc.account_name}" class="w-full border rounded-md px-2 py-1"></td>
                                <td class="border p-2"><input type="text" name="bank_accounts[${i}][account_no]" value="${acc.account_no}" class="w-full border rounded-md px-2 py-1"></td>
                                <td class="border p-2"><input type="text" name="bank_accounts[${i}][branch]" value="${acc.branch ?? ''}" class="w-full border rounded-md px-2 py-1"></td>
                                <td class="border p-2"><input type="text" name="bank_accounts[${i}][swift]" value="${acc.swift ?? ''}" class="w-full border rounded-md px-2 py-1"></td>
                                <td class="border p-2 text-center">
                                    <button type="button" class="removeEditRow bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded">X</button>
                                </td>
                            </tr>
                        `;
                        $('#editBankTableBody').append(row);
                    });
                }
            }
        });
        ...
        let updateUrl = "{{ route('role.suppliers.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'supplier' => '__id__']) }}";
        $('#editForm').attr('action', updateUrl.replace('__id__', supplierId));
    ...
    // Close modal
    $(document).on('click', '.modal-close-edit', function () {
        $('#editModal').addClass('hidden');
    });
