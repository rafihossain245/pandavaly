    ...
    <div class="modal-container bg-white w-11/12 md:max-w-lg mx-auto rounded shadow-lg z-50 overflow-y-auto" style="max-height: 90vh;">
        <div class="modal-content py-4 text-left px-6">
            <div class="modal-header flex justify-between items-center pb-3">
                <h3 class="text-xl font-semibold">Edit Combo Deal</h3>
                <button class="modal-close-edit z-50"><i class="fas fa-times"></i></button>
            </div>
            <div class="modal-body">
                <form id="editForm" method="POST" enctype="multipart/form-data"
                    action="{{ route('role.combo-deals.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'combo_deal' => 1]) }}">
                    @csrf
                    @method('PUT')
                    <input type="hidden" id="editItemId" name="id">
                    <div class="mb-4">
                        <label for="edit_name" class="block text-gray-700 text-sm font-bold mb-2">Combo Name</label>
                        <input type="text" id="edit_name" name="name" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md">
                    </div>
                    <div class="mb-4">
                        <label for="edit_description" class="block text-gray-700 text-sm font-bold mb-2">Description (optional)</label>
                        <textarea id="edit_description" name="description" rows="2" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md"></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="edit_image" class="block text-gray-700 text-sm font-medium mb-2">Image</label>
                        <input type="file" id="edit_image" name="image" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md">
                    </div>
                    <div class="mb-4">
                        <label for="edit_product_ids" class="block text-gray-700 text-sm font-bold mb-2">Products in this combo (pick at least 2)</label>
                    ...
                    <div class="mb-4">
                        <label for="edit_price" class="block text-gray-700 text-sm font-bold mb-2">Combo Price (৳)</label>
                        <input type="number" step="0.01" min="0" id="edit_price" name="price" class="form-input w-full px-3 py-2 border border-gray-300 rounded-md">
                    </div>
            ...
            <div class="modal-footer flex justify-end pt-2">
                ...
                <button data-action="{{ route('role.combo-deals.update', ['role' => Str::slug(Auth::user()->getRoleNames()->first()), 'combo_deal' => 1]) }}"
